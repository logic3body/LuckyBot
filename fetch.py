...
async def cmd_run():
    ...
    extra = getattr(user_config, "TARGET_UIDS", [])
    uids.extend(extra)
    ...
    for uid in uids:
        ...
        if not dynamics:
            print(f"  UID {uid}: 没有新的动态")
        else:
            print(f"  UID {uid}: 获取到 {len(dynamics)} 条动态")
            uid_dynamics_list.append((uid, dynamics))

        # UID 间间隔，避免风控
        if len(uid_dynamics_list) < len(uids):
            await asyncio.sleep(random.uniform(3, 6))

    if not uid_dynamics_list:
        print("所有 UID 都没有新的动态，退出")
        log_action("run", "", uids[0], "skipped", "no_new_dynamics")
        return

    # ---- 阶段 2：合并动态、保存分类文件 ----
    all_dynamics = []
    for _, dynamics in uid_dynamics_list:
        all_dynamics.extend(dynamics)

    # 保存 latest.json
    with open(LATEST_FILE, "w", encoding="utf-8") as f:
        json.dump(all_dynamics, f, ensure_ascii=False, indent=2)

    # 全量分类（供 cmd_forward / cmd_interact 使用）
    full_classified = classify_dynamics(all_dynamics)
    ...
    # ---- 阶段 3：逐 UID 参与抽奖 ----
    for uid_idx, (uid, dynamics) in enumerate(uid_dynamics_list):
        # UID 间间隔（最后一个不需要）
        if uid_idx > 0:
            wait = random.uniform(15, 30)
            print(f"\n等待 {wait:.0f} 秒后处理下一个 UID...")
